...
const PlayerSearch: React.FC = () => {
    ...
    const datas: IColumn[] = keyTypePairs.map(item => {
        ...
        const totalRecords = [east.record, south.record, west.record, north.record,].reduce((acc, value) => acc + value, 0);

        return isPercent
            ? {
                key: type,
                total: percentage ? percent(total / totalRecords) : total,
                east: percentage ? values[0] === 0 ? '0%' : percent(values[0] / east.record) : values[0],
                south: percentage ? values[1] === 0 ? '0%' : percent(values[1] / south.record) : values[1],
                west: percentage ? values[2] === 0 ? '0%' : percent(values[2] / west.record) : values[2],
                north: percentage ? values[3] === 0 ? '0%' : percent(values[3] / north.record) : values[3],
            } : {
                key: type,
                total: total,
                east: values[0],
    ...
    return (
        <Row>
            ...
            {playerStatistics.name &&
                <Col span={24}>
                    <Table columns={columns} dataSource={datas} size='small' pagination={false} />
                </Col>
            }
        </Row>
    )
