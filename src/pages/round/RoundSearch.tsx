import { Col, Row, Select, Table, Typography, Tag } from "antd";
import { ColumnsType } from "antd/es/table";
import { getExistDate, getHistoryByDate } from "apis/mahjong";
import dayjs from "dayjs";
import { EDeskType } from "pages/enum";
import { IHistoryRound } from "pages/interface";
...
const RoundSearch: React.FC = () => {
    ...
    const [historyRound, setHistoryRound] = useState<IHistoryRound[]>([]);
    const [selectedRound, setSelectedRound] = useState<IHistoryRound>({
        uid: '',
        createdAt: new Date(),
    ...
    const data: IColumn[] = [
        {
            key: <Text key='roundSearch_data_east'>
                {`${east.name}`}
            </Text>,
            uKey: 0,
            win: east.win,
        ...
        {
            key: <Text key='roundSearch_data_south'>
                {`${south.name}`}
            </Text>,
            uKey: 1,
            win: south.win,
        ...
        {
            key: <Text key='roundSearch_data_west'>
                {`${west.name}`}
            </Text>,
            uKey: 2,
            win: west.win,
        ...
        {
            key: <Text key='roundSearch_data_north'>
                {`${north.name}`}
            </Text>,
            uKey: 3,
            win: north.win,
    ...
    const dateOptions = existDate.map(date => {
        return {
            key: date,
            value: date,
            label: date
